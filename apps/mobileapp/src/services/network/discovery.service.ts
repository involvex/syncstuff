...
class DiscoveryService {
  ...
  async stopDiscovery(): Promise<void> {
    ...
    try {
      // Step 1: Unregister this device from the network FIRST
      try {
        await ZeroConf.stop();
      ...
      // Step 5: Close the native ZeroConf connection
      try {
        await ZeroConf.close();
        logger.debug("ZeroConf connection closed");
      } catch (error) {
        logger.warn("Failed to close ZeroConf:", error);
      }
      ...
      // Set isRunning to false only at the very end
      this.isRunning = false;
      this.listenerHandle = null;
    }
  }
  ...
  async unregisterDevice(): Promise<void> {
    ...
    // Only unregister if discovery is not running
    // (stopDiscovery handles unregistration if it is running)
    if (this.isRunning) {
      logger.debug("Discovery is running, will be unregistered on stop");
      return;
    }
